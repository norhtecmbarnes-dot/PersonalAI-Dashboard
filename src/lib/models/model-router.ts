...
const AVAILABLE_MODELS: ModelInfo[] = [
  ...
  // These are separate from Ollama and require BitNet installation
  {
    ...
    tier: 'local-fast',
    provider: 'bitnet' as any,
    maxTokens: 4096,
    costPerToken: 0,
    available: true,
    capabilities: ['chat'],
    description: 'CPU-optimized 1.58-bit model, runs without GPU',
  },
  {
    ...
    available: true,
    capabilities: ['chat'],
    description: 'Smaller BitNet model for faster CPU inference',
  },
  {
    id: 'bitnet-b1.58-3b',
    ...
    available: true,
    capabilities: ['chat'],
    description: 'Larger BitNet model, better quality on CPU',
  },
  ...
  // Local models (free) - Small models that run on most hardware
  {
    ...
    description: 'Small 4B model, runs on CPU, no GPU needed',
  },
  ...
  // Fast mid-range models
  {
    ...
    description: 'Fast 9B multimodal model - excellent speed/capability balance',
  },
  ...
  // Capable models (require more resources)
  {
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: 'Large 27B multimodal model',
  },
  {
    id: 'lfm2:latest',
    name: 'LFM2',
  ...
  },
  {
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: 'GPT-based 20B model, requires GPU',
  },
  ...
  // Ollama Cloud models (require OLLAMA_API_KEY)
  {
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: 'Massive 397B Qwen 3.5 on Ollama Cloud',
  },
  {
    id: 'qwen3-coder-next',
    ...
    available: true,
    capabilities: ['chat', 'code'],
    description: '79.7B code specialist on Ollama Cloud',
  },
  {
    id: 'qwen3-coder:480b',
    ...
    available: true,
    capabilities: ['chat', 'code'],
    description: '480B code model on Ollama Cloud',
  },
  {
    id: 'glm-5',
    name: 'GLM-5 (Cloud)',
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: '756B GLM-5 on Ollama Cloud, GPT-like',
  },
  {
    id: 'glm-4.7',
    name: 'GLM-4.7 (Cloud)',
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: 'GLM-4.7 on Ollama Cloud',
  },
  {
    id: 'kimi-k2.5',
    ...
    description: '1.1T parameter, Claude-distilled on Ollama Cloud',
  },
  {
    id: 'kimi-k2-thinking',
    ...
    available: true,
    capabilities: ['chat', 'analysis'],
    description: 'Thinking model on Ollama Cloud',
  },
  {
    id: 'deepseek-v3.2',
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: '671B DeepSeek on Ollama Cloud',
  },
  {
  ...
  },
  {
    ...
    available: true,
    capabilities: ['chat', 'analysis'],
    description: '671B thinking model on Ollama Cloud',
  },
  {
    id: 'minimax-m2.5',
    ...
    available: true,
    capabilities: ['chat', 'analysis'],
    description: 'MiniMax on Ollama Cloud',
  },
  {
    ...
    available: true,
    capabilities: ['chat', 'code'],
    description: '675B Mistral on Ollama Cloud',
  },
  {
    ...
    available: true,
    capabilities: ['chat', 'code'],
    description: '24B code agent on Ollama Cloud',
  },
  {
    id: 'gemma3:27b',
    ...
    available: true,
    capabilities: ['chat', 'code'],
    description: '27B Gemma on Ollama Cloud',
  },
  {
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: 'Gemini 3 Flash on Ollama Cloud',
  },

  // Cloud models (paid via other providers)
  {
    id: 'qwen3.5:full',
    name: 'Qwen 3.5 Full (Cloud)',
    tier: 'cloud-thinking',
    provider: 'cloud',
    maxTokens: 128000,
    ...
    available: true,
    capabilities: ['chat', 'code', 'analysis'],
    description: 'Full-size Qwen 3.5 multimodal model in the cloud',
  },
  {
    id: 'glm-5:cloud',
    name: 'GLM-5 Cloud',
...
const TASK_MODEL_MAP: Record<string, ModelTier> = {
  // Tier 1: Housekeeping - Always use smallest fastest model
  heartbeat: 'local-fast',
  scheduled: 'local-fast',
  health_check: 'local-fast',
  security_quick: 'local-fast', // Use fast model for quick scans
  memory_search: 'local-fast', // Memory searches use fast model
...
class ModelRouter {
  ...
  getModel(
    ...
    if (shouldUseCloud && tier === 'cloud-thinking') {
      const cloudModel = this.models.find(
        m => m.provider === 'cloud' && m.tier === 'cloud-thinking' && m.available
      );
      if (cloudModel) {
  ...
  getTaskModel(taskType: string): ModelInfo {
    ...
    return this.models.find(m => m.tier === 'local-fast') || this.models[0];
  }
  ...
  getDocumentModel(): ModelInfo {
    ...
    return (
      this.models.find(m => m.id === 'qwen3.5:9b') ||
  ...
  getOCRModel(): ModelInfo {
    ...
    return (
      this.models.find(m => m.id === 'glm-ocr') || this.models.find(m => m.id === 'glm-4.7-flash')!
    );
  }
  ...
  getEmbeddingModel(): ModelInfo {
    ...
    return (
      this.models.find(m => m.capabilities.includes('embedding')) ||
      this.models.find(m => m.id === 'glm-4.7-flash')!
    );
  }
...
export const trackTokenUsage = (tokens: number, modelId: string) =>
  router.trackTokenUsage(tokens, modelId);
