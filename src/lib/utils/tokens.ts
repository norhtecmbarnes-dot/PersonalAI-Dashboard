...
export class TokenOptimizer {
  private static maxContextTokens = 2048; // Reduced from 4096 for efficiency
  ...
  static compressMessage(message: ConversationMessage): ConversationMessage {
    ...
    const importantPatterns = [/```[\s\S]*?```/g, /`[^`]+`/g, /\*\*[^*]+\*\*/g];
  ...
  static getUsageStats(usage: TokenUsage): {
    ...
    return {
      ...
      costEstimate:
        (usage.prompt / 1000) * promptCostPer1k + (usage.completion / 1000) * completionCostPer1k,
    };
  }
}
