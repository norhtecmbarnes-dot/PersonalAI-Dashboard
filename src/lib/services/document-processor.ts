...
export class DocumentProcessor {
  ...
  private async processPDF(file: File): Promise<string> {
    try {
      ...
      try {
        ...
        console.warn('pdf-parse failed, using fallback:', e);
      }

      return `[PDF Document: ${file.name}\nSize: ${(file.size / 1024).toFixed(1)} KB\nPages: Unknown\n\nNote: Full text extraction failed. The PDF may be encrypted, corrupted, or contain only images.]`;
    } catch (error) {
      console.error('PDF processing error:', error);
      return `[PDF Error: Could not read ${file.name}]`;
    }
  }
  ...
  compactContent(content: string, targetLength: number = 8000): string {
    ...
    if (compacted.length > targetLength) {
      compacted =
        compacted.substring(0, targetLength - 100) +
