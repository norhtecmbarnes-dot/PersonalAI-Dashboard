...
class ToolRegistry {
  ...
  private registerBuiltInTools(): void {
    ...
    this.registerTool({
      name: 'document_list',
      description:
        'List uploaded documents. Use when user wants to see what documents are available.',
      parameters: {},
      execute: async () => {
        try {
          const { sqlDatabase } = await import('@/lib/database/sqlite');
        ...
        try {
          ...
          if (response.ok && data.success) {
            ...
            if (extracted.contacts?.length)
              summary += `- ${extracted.contacts.length} contact(s)\n`;
            ...
            if (extracted.phones?.length)
              summary += `- ${extracted.phones.length} phone number(s)\n`;
    ...
    this.registerTool({
      ...
      description:
        'Track a SAM.gov opportunity for monitoring. Use this when the user wants to add a government contracting opportunity to their watch list.',
      parameters: {
        title: { type: 'string', description: 'Opportunity title', required: true },
        solicitationNumber: {
          type: 'string',
          description: 'Solicitation number (e.g., FA8652-26-R-0001)',
          required: true,
        },
        type: {
          type: 'string',
          description: 'Opportunity type (Solicitation, Presolicitation, Sources Sought, etc.)',
          required: false,
        },
        ...
        responseDeadline: {
          type: 'string',
          description: 'Due date/time (ISO format)',
          required: false,
        },
    ...
    this.registerTool({
      ...
      description:
        'List tracked SAM.gov opportunities. Use this when the user wants to see their tracked opportunities.',
      parameters: {
        status: {
          type: 'string',
          description: 'Filter by status: active, past_due, awarded, closed',
          required: false,
        },
      },
      execute: async params => {
          ...
          if (response.ok) {
            ...
            return {
              ...
              data: {
                ...
                message:
                  opps.length > 0
                    ? `Found ${opps.length} tracked opportunity(ies)`
                    : 'No tracked opportunities found.',
              },
            };
          }
    ...
    this.registerTool({
      ...
      parameters: {
        solicitationNumber: {
          type: 'string',
          description: 'Solicitation number to find the opportunity',
          required: true,
        },
        status: {
          type: 'string',
          description: 'New status: active, past_due, awarded, closed',
          required: true,
        },
      ...
      execute: async params => {
        try {
          ...
          const findResponse = await fetch(
            `/api/sam/track?solicitationNumber=${encodeURIComponent(params.solicitationNumber)}`
          );
          ...
          if (!findData.opportunity) {
            return {
              success: false,
              error: `Opportunity ${params.solicitationNumber} not found in tracked list`,
            };
          }
