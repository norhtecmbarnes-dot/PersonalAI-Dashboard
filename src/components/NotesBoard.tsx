      ...
      try {
        const response = await fetch('/api/database', {
          ...
          body: JSON.stringify({
            ...
            data: {
              id,
              updates: { positionX, positionY, width: width || 300, height: height || 200 },
            },
          }),
        });
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      if (dragState.noteId) {
        ...
        setNotes(prev =>
          prev.map(note =>
            note.id === dragState.noteId ? { ...note, positionX: newX, positionY: newY } : note
          )
        );
      }
      ...
      if (resizeState.noteId) {
        ...
        setNotes(prev =>
          prev.map(note =>
            note.id === resizeState.noteId ? { ...note, width: newWidth, height: newHeight } : note
          )
        );
      }
    },
      ...
      {notes.length === 0 ? (
        ...
        notes.map(note => {
          ...
          return (
            <div
              key={note.id}
              className={`absolute cursor-move shadow-lg hover:shadow-xl transition-shadow ${colorStyle.bg} ${colorStyle.border} border rounded-sm overflow-hidden ${isSelected ? 'ring-2 ring-slate-800 ring-offset-2 ring-offset-slate-900' : ''}`}
              style={{
            ...
            >
              {/* Pin */}
              <div
                className={`absolute -top-1.5 left-1/2 transform -translate-x-1/2 w-4 h-4 rounded-full ${colorStyle.pin}`}
              ...
              >
                <input
                  ...
                  onChange={e => {
                    ...
                    setNotes(prev =>
                      prev.map(n => (n.id === note.id ? { ...n, title: newTitle } : n))
                    );
                  }}
                  onBlur={async () => {
              ...
              <div
                className={`absolute bottom-0 left-0 right-0 px-2 py-1 flex items-center gap-1 ${colorStyle.bg} border-t ${colorStyle.border}`}
              >
                ...
                {note.tags?.slice(0, 2).map((tag: string) => (
                  <span
                    key={tag}
                    className={`text-xs px-1.5 py-0.5 ${colorStyle.text} bg-black/10 rounded truncate max-w-[60px]`}
                  >
                    {tag}
                  </span>
                ))}
