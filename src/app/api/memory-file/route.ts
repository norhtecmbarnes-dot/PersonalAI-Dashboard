    ...
    switch (action) {
      case 'memory':
        try {
          ...
          sqlDatabase.setSetting('memory_file', JSON.stringify(body.memory));
          return NextResponse.json({ success: true });
        } catch (e) {
          console.error('[MemoryFile] Could not save memory:', e);
          return NextResponse.json(
      ...
      case 'updateContext':
        await memoryFileService.updateContext(
          ...
          (body.activeQueries || [])
            .slice(0, 10)
            .map((q: string) => sanitizeString(q).slice(0, 200))
        );
        return NextResponse.json({ success: true });
      ...
      case 'updateSoul':
        try {
          ...
          return NextResponse.json(
            { success: false, error: 'Failed to save soul' },
            { status: 500 }
          );
        }
